...
function getStudentAverage(students) {
    ...
    let classAverage=0;
        for( let name of students){
            ...
            for(let grades of name.grades){
                average+=grades
                
            }
            students["average"]=average/name.grades.length
            
            avgstudents.push({name: name.name, average: students.average})
     
            average=0
            
        }
        
        for(let each of avgstudents){
            classAverage+=each.average
            
            }
            classAverage=classAverage/avgstudents.length
            avgstudents.unshift({classAverage: parseFloat(classAverage.toFixed(2))})
            return avgstudents
}


console.log( Input= getStudentAverage([
  {
    name: "Pedro",
    grades: [90, 87, 88, 90],
  },
